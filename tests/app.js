...
/* ===== Expansion topics: wrapper gens keep Knowledge-Map attribution clean ===== */
function genWMAnalogy(){ return Math.random()<0.12 ? choice(POOL.analogies) : genAnalogy(); }
function genWMSynonym(){ return choice(POOL.synonyms); }
function genWMAnalogy2(){ return genAnalogy(); }
function genSPRates(){ return choice([genAvgSpeed, genPipesTogether, genTwoVarWord])(); }
function genSPNumTheory(){ return choice([genUnitsDigit, genRemainder, genLcm, genCustomOp])(); }
function genSPAlgebra(){ return choice([genSystemEq, genFactoredQuad, genFunctionEval, genExponentQuotient])(); }
function genSPGeo(){ return choice([genRectRatioArea, genPolygonAngles, genRectDiagonal])(); }
TOPICS.push(
  { id:'wm_ana1', verbal:true, title:'Analogy Foundations', sub:'bridge sentences and relationship types',
    gens:[genWMAnalogy],
    lesson:`<p><b>Every analogy is a hidden sentence.</b> CUB is to BEAR isn't about animals — it's the sentence "a cub is a young bear." Find that sentence and the answer finds itself.</p>
      <p><b>The bridge method:</b> build your sentence from the FIRST pair before you even look at the choices. Then test each choice in the exact same sentence. "A kitten is a young cat" — fits. "A dog is a young kennel" — nonsense. Cross it off.</p>
      <div class="passage"><b>Worked example:</b> QUILL is to PORCUPINE. Bridge: "a quill is the sharp covering of a porcupine." Now test: THORN is to ROSE — "a thorn is the sharp covering of a rose." Fits perfectly.</div>
      <p><b>The big relationship families</b> you'll see again and again: part/whole (petal : flower), worker/tool (surgeon : scalpel), young/adult (foal : horse), degree (warm : scorching), object/function (knife : cut), and word/opposite (scarce : plentiful).</p>
      <p><b>The trap to expect:</b> one answer will share a TOPIC with the question (bear → another animal) but not the RELATIONSHIP. Topics are bait. Relationships are the answer.</p>
      <p>Precision beats vibes: if two choices seem to fit, make your bridge sentence more specific — "a cub is a <i>young</i> bear," not "a cub is related to a bear."</p>`},
  { id:'wm_syn', verbal:true, title:'Synonyms & Roots', sub:'word parts that unlock meanings',
    gens:[genWMSynonym],
    lesson:`<p><b>You know more words than you think</b> — because English words are built from parts you've already met.</p>
      <p><b>Prefixes flip or aim a word:</b> BEN- means good (benevolent, benefit). MAL- means bad (malice, malfunction). CIRCUM- means around (circumference). Spot the prefix and you have half the meaning.</p>
      <p><b>Roots carry the core:</b> DICT = say (predict, dictate, contradict). SPEC = look (inspect, spectator, spectacle). CRED = believe (credible, incredulous). PORT = carry (transport, portable).</p>
      <div class="passage"><b>Worked example:</b> INCREDULOUS. IN- (not) + CRED (believe) = "not believing." So when the test asks for the closest meaning, you're hunting for <i>skeptical</i> — even if you've never seen the word before.</div>
      <p><b>Elimination wins ties:</b> on the SSAT, wrong choices are often the right word's OPPOSITE, a sound-alike (statue/statute), or a word from the same setting but different meaning. Kill those three and guess from what's left — the odds are now with you.</p>
      <p><b>Charge check:</b> decide if the word FEELS positive or negative before defining it. LOATHE feels negative — so cross out every positive choice instantly.</p>`},
  { id:'wm_ana2', verbal:true, title:'Advanced Analogies', sub:'degree, function, and lack',
    gens:[genWMAnalogy2],
    lesson:`<p>The hardest analogies use <b>subtle relationships</b> — here are the three that decide top scores.</p>
      <p><b>1. Degree:</b> both words mean the same thing at different intensity. ANNOYED is to FURIOUS as DAMP is to SOAKED. The bridge: "___ is a much stronger version of ___." Watch the direction — mild to extreme is not extreme to mild.</p>
      <p><b>2. Function:</b> what a thing is FOR. ANCHOR is to MOOR as RULER is to MEASURE. Bridge: "the purpose of an ___ is to ___."</p>
      <p><b>3. Lack:</b> the sneakiest — one word means the ABSENCE of the other. ARID is to MOISTURE as NAIVE is to EXPERIENCE. Bridge: "something ___ lacks ___." Kids who miss these read them as synonyms; they're closer to opposites.</p>
      <div class="passage"><b>Worked example:</b> PENNILESS is to MONEY. Not "penniless people want money" (too loose). The tight bridge: "penniless means lacking money." Test: ANONYMOUS is to NAME — "anonymous means lacking a name." Locked in.</div>
      <p><b>Direction discipline:</b> always test choices in the same order as the stem. Reversing the order is the #1 advanced-analogy error — the test writers know it and set traps for it.</p>`},
  { id:'sp_rates', title:'Rates, Work & Motion', sub:'speed, together-work, and two travelers',
    gens:[genSPRates],
    lesson:`<p><b>Every rate problem is the same equation wearing different clothes:</b> amount = rate × time. Distance problems, painting-a-fence problems, filling-a-pool problems — all of them.</p>
      <p><b>Average speed is the famous trap:</b> it is NOT the average of the speeds. It's total distance ÷ total time, always. Drive somewhere at 30 and back at 60? The slow half eats more time, so the answer is 40, not 45.</p>
      <div class="passage"><b>Worked example:</b> A pipe fills a tank in 6 hours; another in 3. Together? Think per-hour: pipe one does 1/6 per hour, pipe two does 1/3. Together 1/6 + 1/3 = 1/2 per hour → 2 hours total.</div>
      <p><b>The together-work rule:</b> add the RATES, never the times. Two workers never take longer together than the faster one alone — use that to sanity-check your answer.</p>
      <p><b>Two-traveler setups:</b> moving toward each other, their speeds ADD (closing the gap fast). Same direction, SUBTRACT (the gap closes slowly). Draw the arrows before you compute anything.</p>`},
  { id:'sp_numth', title:'Number Detective', sub:'remainders, cycles, and hidden patterns',
    gens:[genSPNumTheory],
    lesson:`<p>Top-2% problems love numbers too big to compute — <b>because they're testing whether you hunt the pattern instead.</b></p>
      <p><b>Units digits run in cycles.</b> Powers of 7 end in 7, 9, 3, 1, 7, 9, 3, 1... forever, repeating every 4. So the units digit of 7^41? 41 = 4×10 + 1 → first slot of the cycle → 7. You never compute 7^41; you compute 41 mod 4.</p>
      <div class="passage"><b>Worked example:</b> What's the remainder when 100 is divided by 7? 7×14 = 98, so remainder 2. Remainders are just "what's left after the biggest multiple that fits."</div>
      <p><b>LCM = when cycles line up.</b> Buses every 6 and every 8 minutes leave together — next time together? LCM(6,8)=24. Any "when do they meet again" problem is an LCM problem in a costume.</p>
      <p><b>Made-up operators</b> (a ◆ b = 2a + b or whatever the test invents) are pure instruction-following: substitute slowly, compute inside-out, and never let the weird symbol scare you — it's defined right there in the problem.</p>`},
  { id:'sp_alg', title:'Algebra Boss Fights', sub:'systems, quadratics, and function notation',
    gens:[genSPAlgebra],
    lesson:`<p>Three algebra bosses guard the top scores. Each has a clean opening move.</p>
      <p><b>Systems (two equations):</b> the opening is elimination — stack the equations and add or subtract to kill a variable. x+y=10, x−y=4 → add: 2x=14 → x=7, y=3. If adding doesn't kill anything, multiply one equation first.</p>
      <p><b>Factored quadratics:</b> if (x−3)(x+5)=0, then x=3 or x=−5 — because anything times zero is zero. The test's favorite follow-up: "what is the SUM of the solutions?" 3+(−5)=−2. Answer the question they asked, not the one you expected.</p>
      <div class="passage"><b>Worked example:</b> f(x) = 3x − 2. What is f(4)? Function notation is a machine: feed in 4 wherever x appears → 3(4) − 2 = 10. f(4) is NOT f times 4 — it's "the machine's output when the input is 4."</div>
      <p><b>Exponent quotients:</b> same base dividing → subtract exponents. 5^9 ÷ 5^6 = 5^3 = 125. Multiplying → add. Power of a power → multiply. Three rules, no exceptions, worth ten points a season.</p>`},
  { id:'sp_geo', title:'Geometry Masters', sub:'diagonals, polygons, and ratio areas',
    gens:[genSPGeo],
    lesson:`<p>Advanced geometry is spotting <b>the hidden simple shape inside the fancy one.</b></p>
      <p><b>Every rectangle hides two right triangles.</b> A diagonal question IS a Pythagorean question: 6 by 8 rectangle → diagonal² = 36+64 = 100 → 10. Memorize the triples (3-4-5, 5-12-13, 8-15-17) and these become ten-second points.</p>
      <p><b>Polygon interior angles:</b> (n−2)×180. A hexagon: 4×180 = 720. Why it works: any polygon splits into (n−2) triangles from one corner, and each triangle contributes 180°.</p>
      <div class="passage"><b>Worked example:</b> Two rectangles have the same height, but one is 3× as wide. Areas? Same height means area scales exactly with width — 3× the area. Ratio problems about area often never need actual numbers.</div>
      <p><b>The master habit:</b> when a figure looks new, draw one line — a diagonal, a height, a radius — and watch it fall apart into triangles and rectangles you already own.</p>`}
);
COURSES.push(
  { id:'words',  title:'Word Mastery',    desc:'The verbal section, taught — analogies, roots, and the traps', topics:['wm_ana1','wm_syn','wm_ana2'] },
  { id:'summit', title:'Summit Problems', desc:'The hardest material on the test — where top scores are made', topics:['sp_rates','sp_numth','sp_alg','sp_geo'] }
);
...
function renderNextUp(){
  ...
  let mode, title, sub;
  if (!account.diagAt){
    mode='gaps'; title='Find your gaps'; sub='the full diagnostic maps all four skills';
...
function ensureFields(a){
  ...
  if(typeof a.obsidianSeen!=='boolean') a.obsidianSeen=false;
}
...
function startFullTest(){
  if (!isPremium() && account && !account.freeTestUsed){ account.freeTestUsed=true; saveStore(); showToast('Your free Full Test — make it count!'); }
...
function renderOnlineHome(msg){
  ...
  if (oMe) setTimeout(()=>osend({t:'friends'}), 50);
  o.innerHTML=`
  <div class="card">
    ...
    ${oMe ? `
      ...
      <div class="mlabel">Tournaments</div>
      <div id="tourneyBox">
        <div class="tier-row" style="cursor:pointer" onclick="osend({t:'tourney_join'}); showToast('Looking for 4 players…')"><span><b>${ico('trophy',14)} Bracket</b><br><span style="font-weight:400;font-size:12px;color:var(--gray)">4 players, seeded by rating — semifinals, then the final</span></span><span class="tier-rp">Join &rarr;</span></div>
        <div class="tier-row" style="cursor:pointer" onclick="arenaActive=true; osend({t:'arena_join'}); showToast('Arena joined — races start when an opponent arrives')"><span><b>${ico('flame',14)} Arena</b><br><span style="font-weight:400;font-size:12px;color:var(--gray)">10 minutes, quick races to 5 — wins score points, most points takes it</span></span><span class="tier-rp">Join &rarr;</span></div>
        <div id="tourneyStatus"></div>
      </div>
...
function obsidianQualified(){ return !!(oMe && oMe.rating>=1200) || !!(account && account.onlineRating>=1200); }
function boardHTML(){
  let rows;
  if (acctBoard==='obsidian'){
    rows = (lastBoard||[]).map((u,i)=>`<div class="tier-row ${oMe&&u.name===oMe.name?'tier-on':''}" style="border-left:3px solid #3a3060"><span>${i+1}. ${u.flair?ico('crown',12)+' ':''}${esc(u.name)}</span><span class="tier-rp">${u.rating!=null?u.rating:'&mdash;'} &middot; ${u.wins}W&ndash;${u.losses}L</span></div>`).join('')
      || '<p class="section-instr">The chamber is empty tonight&hellip;</p>';
    ...
    rows = `<p class="section-instr" style="margin-top:0">⬢ The Obsidian League — rating 1200+. Few enter. Fewer stay.</p>` + rows;
  }
  else if (acctBoard==='friends'){
  ...
  return `
    <div class="seg-bar" style="margin:0 0 8px">
      <button class="seg ${acctBoard==='global'?'seg-on':''}" onclick="acctBoard='global'; osend({t:'leaderboard'}); renderFriendsPage()">Global top 10</button>
      <button class="seg ${acctBoard==='friends'?'seg-on':''}" onclick="acctBoard='friends'; renderFriendsPage()">Friends</button>
      ${obsidianQualified()?`<button class="seg ${acctBoard==='obsidian'?'seg-on':''}" style="color:#5a4d8a" onclick="acctBoard='obsidian'; osend({t:'leaderboard', min:1200}); renderFriendsPage()">⬢ Obsidian</button>`:''}
    </div>${rows}`;
}
...
let findMatchLen=11, arenaActive=false;
function addFriend(){
...
function onlineMsg(m){
  if (m.t==='auth'){
    ...
    document.getElementById('online').innerHTML=`
      <div class="card" style="text-align:center">
        ...
        <p class="section-instr">${m.label?`<b style="color:var(--gold)">${esc(m.label)}</b> &middot; `:''}You vs <b>${esc(oOpp.name)}</b>${oOpp.rating!=null?` (${oOpp.rating} rating)`:''}. First to ${m.winPoints} points. Get ready&hellip;</p>
      </div>`;
  }
  else if (m.t==='round'){
    ...
    if (account && m.rating>=1200 && !account.obsidianSeen){ account.obsidianSeen=true; saveStore(); celebrate(); showToast('⬢ You have been noticed. The Obsidian League is open to you — see My Account.'); }
    if(m.score) oScore=m.score;
  ...
  else if (m.t==='tourney_wait'){
    const s=document.getElementById('tourneyStatus');
    if (s) s.innerHTML = m.n>0 ? `<p class="section-instr">Bracket lobby: ${m.n}/${m.need} players — waiting for challengers&hellip;</p>` : '';
  }
  else if (m.t==='tourney_start'){
    playFanfare();
    const s=document.getElementById('tourneyStatus');
    const br=(m.bracket||[]).map(b=>`#${b.seed} ${esc(b.name)}${b.rating!=null?' ('+b.rating+')':''}`).join(' &middot; ');
    if (s) s.innerHTML=`<div class="explain ok">Tournament on! You are seed #${m.seed}. ${br}</div>`;
    showToast('Tournament starting — you are seed #'+m.seed);
  }
  else if (m.t==='tourney_round'){
    showToast(m.msg||'The final is forming…');
  }
  else if (m.t==='tourney_result'){
    if (m.yours){ celebrate(); showToast('🏆 CHAMPION! You won the tournament!'); }
    else showToast('Tournament over — champion: '+esc(m.champion||''));
    const s=document.getElementById('tourneyStatus');
    if (s) s.innerHTML=`<div class="explain ${m.yours?'ok':''}">Champion: <b>${esc(m.champion||'')}</b>${m.yours?' — that\'s YOU ✦':''}</div>`;
  }
  else if (m.t==='arena_state'){
    const s=document.getElementById('tourneyStatus');
    if (s){
      const rows=(m.top||[]).map((u,i)=>`${i+1}. ${esc(u.name)} — ${u.pts}`).join('<br>');
      s.innerHTML=`<div class="explain"><b>Arena &middot; ${fmtTime(m.secs)} left</b><br>${rows||'first race pending…'}</div>`;
    }
  }
  else if (m.t==='arena_next'){
    if (arenaActive && m.secs>3) setTimeout(()=>{ if(arenaActive && oState==='idle') osend({t:'arena_join'}); }, 2500);
  }
  else if (m.t==='arena_result'){
    arenaActive=false;
    playFanfare();
    showToast('Arena over — champion: '+esc(m.champion||'nobody'));
    const s=document.getElementById('tourneyStatus');
    if (s) s.innerHTML=`<div class="explain ok"><b>Arena final standings</b><br>${(m.top||[]).map((u,i)=>`${i+1}. ${esc(u.name)} — ${u.pts} pts`).join('<br>')}</div>`;
  }
  else if (m.t==='friends'){
  ...
  else if (m.t==='challenge_result'){
    showToast(m.msg || (m.ok?'Challenge sent!':'Challenge failed'));
  }
  else if (m.t==='challenged'){
    const o=document.getElementById('online');
    const vis = o && !o.classList.contains('hidden');
    playCoachChime();
    if (vis){
      const box=document.getElementById('friendBox');
      const bar=document.createElement('div');
  ...
  el.innerHTML=`
    <div class="card">
      ...
      ${store.diagState && store.diagState.idx>0 && store.diagState.idx<store.diagState.items.length
        ...
        : account.diagAt
        ? `<button class="btn secondary" onclick="startDiag()">Retake the diagnostic (${TOPICS.filter(t=>!t.verbal).length*2+18} questions)</button>`
        : `<button class="btn" onclick="startDiag()">Start the diagnostic (${TOPICS.filter(t=>!t.verbal).length*2+18} questions)</button>`}
...
function buildDiagItems(){
  const items=[];
  TOPICS.forEach((t,i)=>{ if(t.verbal) return; for(let k=0;k<2;k++){
...
const CHANGELOG = [
  ['0.31.0','Two new courses (Word Mastery + Summit Problems), the hidden Obsidian League, and online tournaments: Arena and seeded Brackets'],
...
function appPlatform(){
  if (window.webkit && window.webkit.messageHandlers) return /iPhone|iPad|iPod/.test(navigator.userAgent) ? 'iOS app' : 'Mac app';
...
/* ===== Native store bridge (iOS shell): real StoreKit checkout ===== */
const STORE_PRODUCTS = { solo:'app.quizard.solo.season', unlimited:'app.quizard.unlimited.season', family:'app.quizard.family.season' };
let livePrices = null;
function nativeStore(){ return (window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.buy) ? window.webkit.messageHandlers : null; }
function applyPlan(plan){
  account.premium = true; account.premiumPlan = plan;
  if (plan==='family'){
    store.familyPremium = true;
    if (!account.onlineToken || !ows || ows.readyState!==1) ensureOnlineIdentity();
    setTimeout(()=>osend({t:'family_create'}), 800);
  }
  saveStore();
  celebrate();
...
window.quizardPurchase = function(res){
  if (res && res.ok && res.plan){ applyPlan(res.plan); return; }
  if (res && res.cancelled) return;
  if (res && res.pending){ showToast(res.msg||'Waiting for a parent to approve'); return; }
  showToast((res && res.msg) || 'Purchase did not go through');
};
window.quizardRestore = function(plans){
  if (plans && plans.length){
    const plan = plans.includes('family') ? 'family' : plans.includes('unlimited') ? 'unlimited' : 'solo';
    applyPlan(plan);
    showToast('Purchases restored');
  } else showToast('No purchases found for this Apple ID');
};
window.quizardPrices = function(p){ livePrices = p; if (typeof openPremium==='function'){ const el=document.getElementById('premium'); if (el && !el.classList.contains('hidden')) openPremium(); } };
...
function openPremium(){
  ...
  if (native && !livePrices) native.prices.postMessage('');
  document.getElementById('intro').classList.add('hidden');
      ...
      : `
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:10px;margin:12px 0 6px">
          <div style="border:1.5px solid var(--line);border-radius:12px;padding:14px 10px;text-align:center">
            ...
            <div style="margin:6px 0"><span style="font-size:26px;font-weight:800;color:var(--navy)">${livePrices&&livePrices.solo?livePrices.solo:'$79.99'}</span><span style="color:var(--gray);font-size:12px"> / 3 mo</span></div>
          ...
          <div style="border:1.5px solid var(--gold);border-radius:12px;padding:14px 10px;text-align:center;position:relative">
            ...
            <div style="margin:6px 0"><span style="font-size:26px;font-weight:800;color:var(--navy)">${livePrices&&livePrices.family?livePrices.family:'$349.99'}</span><span style="color:var(--gray);font-size:12px"> / 3 mo</span></div>
        ...
        ${nativeStore() ? `<p class="section-instr" style="text-align:center;margin-top:6px">Billed through the App Store. Auto-renews each season; cancel anytime in Settings. <a href="https://spaceballtwo.github.io/quizard/privacy.html" style="color:var(--amber)">Privacy</a></p>`
...
function restorePurchases(){
  if (!account){ showToast('Log into an account first'); return; }
  const native = nativeStore();
  if (native){ native.restore.postMessage(''); return; }
  if (!account.onlineToken || !ows || ows.readyState!==1){ ensureOnlineIdentity(); }
...
function buyPremium(plan){
  if (!account){ showToast('Log into an account first'); return; }
  const native = nativeStore();
  if (native){ native.buy.postMessage(STORE_PRODUCTS[plan||'solo']); return; }   // Apple checkout; result arrives via quizardPurchase
...
function openReport(){
  ...
  el.innerHTML=`
    <div class="card">
      ...
      ${!isPremium()
        ? `<div class="tier-row" style="border-left:4px solid var(--gold)"><span><b>The written story</b><br><span style="font-weight:400;font-size:12px;color:var(--gray)">an AI-drafted narrative of wins, gaps, and this week's plan — plus season-long score tracking</span></span><span class="tier-rp">${ico('crown',14)}</span></div>
           <button class="btn" onclick="navTo('premium')">Go Premium</button>`
...
/* ===== Ambient background: drifting glows + twinkling sparkles, theme-aware ===== */
(function bgfx(){
  ...
  function draw(){
    ...
    blobs.forEach((b,i)=>{
      b.a += b.sp;
      const bx = b.ox*W + Math.cos(b.a)*W*0.06, by = b.oy*H + Math.sin(b.a*0.8)*H*0.05;
      const g = x.createRadialGradient(bx,by,0,bx,by,b.r);
      g.addColorStop(0, t.blobs[i]); g.addColorStop(1, 'rgba(0,0,0,0)');
      x.fillStyle = g; x.fillRect(0,0,W,H);
    });
    orbs.forEach(o=>{
      o.ph += o.tw; o.py += o.dy;
      if (o.py < -0.06){ o.py = 1.06; o.px = Math.random(); }
      const oa = 0.10+0.22*Math.abs(Math.sin(o.ph));
      const g = x.createRadialGradient(o.px*W, o.py*H, 0, o.px*W, o.py*H, o.r);
      g.addColorStop(0, t.spark); g.addColorStop(1, 'rgba(0,0,0,0)');
      x.globalAlpha = oa; x.fillStyle = g;
      x.beginPath(); x.arc(o.px*W, o.py*H, o.r, 0, 7); x.fill();
      x.globalAlpha = 1;
    });
    sparks.forEach(s=>{
      ...
      x.beginPath();
      x.moveTo(px, py-r*2); x.quadraticCurveTo(px+r*0.35, py-r*0.35, px+r*2, py);
      x.quadraticCurveTo(px+r*0.35, py+r*0.35, px, py+r*2);
      x.quadraticCurveTo(px-r*0.35, py+r*0.35, px-r*2, py);
      x.quadraticCurveTo(px-r*0.35, py-r*0.35, px, py-r*2);
      x.fill();
      x.globalAlpha = 1;
    });
  ...
  (function loop(){
    if (frame % 2 === 0) draw();   // ~30fps — plenty for ambience, kind to batteries
    else frame++;
